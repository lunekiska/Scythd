package eu.lunekiska.scythd.compat.utils;
...
public class BotaniaTerraReapBlocks extends HoeItem implements IManaUsingItem, ISortableTool
{
    private static final int MANA_PER_DAMAGE = 100;

    public BotaniaTerraReapBlocks(Properties props, int attackDamage)
    {
        this(BotaniaAPI.instance().getManasteelItemTier(), attackDamage, props, -1f);
    }

    public BotaniaTerraReapBlocks(IItemTier mat, int attackDamage, Properties properties, float attackSpeed)
    {
        super(mat, (int) -mat.getAttackDamage(), attackSpeed, properties);
    }

    public <T extends LivingEntity> int damageItem(ItemStack stack, int attackDamage, int amount, T entity, Consumer<T> onBroken)
    {
        ...
        for (int x = -radius; x <= radius; ++x)
        {
            for (int y = -1; y <= 1; ++y)
            {
                for (int z = -radius; z <= radius; ++z) {
                    ...
                    int getManaPerDamage = MANA_PER_DAMAGE;
                    ...
                    if (getManaPerDamage < 50)
                    {
                        ...
                        user.getHeldItemMainhand().damageItem(getManaPerDamage, (LivingEntity) user, ((e) -> e.sendBreakAnimation(EquipmentSlotType.MAINHAND)));
